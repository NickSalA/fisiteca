...
bool verificarCredenciales(const wstring& usuario, const wstring& contraseña) {
    ...
    if (archivo.fail()) {
        wcout << L"Error: No se pudo abrir el archivo 'credUsuario.txt'." << endl;
        return false;
    }
...
int main() {
    ...
    do {
        ...
        gotoxy(x + 7, y + 10); wcout << L"¡Bienvenido, " << usuarioIngresado << L"! Has ingresado correctamente." << endl << endl;
